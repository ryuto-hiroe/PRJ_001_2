...
bool EditorStage::Create()
{
	...
	for (int i = 0; i < (int)m_gridSize; i++)
	{
...
{
	SHMGR.m_PrimSh.DrawLine(m_LineR, { 1, 0, 0, 1 });
	SHMGR.m_PrimSh.DrawLine(m_LineG, { 0, 1, 0, 1 });
	SHMGR.m_PrimSh.DrawLine(m_LineB, { 0, 0, 1, 1 });
	
	for (auto&& model : m_LineX)
	{
		SHMGR.m_PrimSh.DrawLine(model, { 1, 1, 1, 0.7f });
	}
